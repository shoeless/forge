...
public class ConquestAEtherScreen extends FScreen {
    ...
    private abstract class AbstractFilterButton<T> extends FLabel implements Predicate<PaperCard> {
        ...
        private AbstractFilterButton(String caption0, T[] options0, final FSerializableFunction<T, String> display) {
            ...
            setCommand(e -> GuiChoose.getChoices(Forge.getLocalizer().getMessage("lblSelectCaptionFilter", caption), 0, 1, options, java.util.Collections.singleton(selectedOption), display, result -> {
                if (!result.isEmpty()) {
