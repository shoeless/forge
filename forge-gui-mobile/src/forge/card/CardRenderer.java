...
public class CardRenderer {
    ...
    public static void drawCard(Graphics g, CardView card, float x, float y, float w, float h, CardStackPosition pos, boolean rotate, boolean showAltState, boolean isChoiceList, boolean magnify) {
        // DEFENSIVE (multiplayer): a network client can hold an ID-only CardView stub whose
        // Current/Alternate state was never synced (e.g. a permanent bounced into a hidden hand
        // by a River's Rebuke-style mass bounce). Render the state-free face-down back instead
        // of NPEing on getImageKey()/getSetCode() every frame on the render thread.
