...
public class DetainAi extends SpellAbilityAi {
    ...
    // Upstream declares this predicate on SpellAbilityAi using the JDK Predicate type;
    // inlined here with the local forge.util.function.Predicate backport (iOS/Java 8).
    private static final Predicate<Card> CREATURE_OR_TAP_ABILITY = c -> {
        if (c.isCreature()) {
            return true;
        }

        for (final SpellAbility ab : c.getSpellAbilities()) {
            if (ab.isAbility() && ab.getPayCosts().hasTapCost()) {
                return true;
            }
        }
        return false;
