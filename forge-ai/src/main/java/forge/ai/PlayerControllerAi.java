...
public class PlayerControllerAi extends PlayerController {
    ...
    public CounterType chooseCounterType(List<CounterType> options, SpellAbility sa, String prompt,
        ...
        // Deterministic tie-break: `options` is typically built from a permanent's getCounters()
        // key set, whose iteration order varies per JVM run (identity hash). The SpellApi AI
        // deciders pick first-on-tie, so an unsorted list makes the chosen counter type
        // nondeterministic across seeded runs. Sort a copy by the stable counter name so the
        // pick is reproducible (rolls no RNG; rules-neutral - only resolves ties, which were
        // arbitrary anyway). One place fixes every CountersRemove/Put/Move consumer here.
        final List<CounterType> sorted = new ArrayList<>(options);
        sorted.sort(java.util.Comparator.comparing(CounterType::toString));
