...
public class AiController {
    ...
    private SpellAbility chooseSpellAbilityToPlayFromList(final List<SpellAbility> all, boolean skipCounter) {
        ...
        // Per-call decline memo: identical token abilities the AI rejects collapse to a single
        // canPlayAndPayFor evaluation. Only declines are cached (a WillPlay returns immediately and
        // carries live mutated targets/X), so the loop's observable result is unchanged. Allocated
        // only when duplicate tokens exist (else null => no signatures computed). Final so the
        // FutureTask lambda below can capture it.
        final java.util.Map<String, Boolean> declinedSigs =
                AiCardSignature.boardHasDuplicateTokens(game) ? new java.util.HashMap<>() : null;
